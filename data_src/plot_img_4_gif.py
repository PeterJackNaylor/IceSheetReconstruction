...
dataset_t = inr.XYTZ_grid(
    XYT_xy,
    0,
    nv_samples=model_hp.nv_samples,
    step_grid=step_grid,
    gpu=gpu,
    temporal=model_hp.nv_samples.shape[0] == 3,
)
grid_xy = (
    dataset_t.samples * model_hp.nv_samples[:, 1] + model_hp.nv_samples[:, 0]
).numpy()
...
mask_resized = transform.resize(
    mask, (y_res, x_res), order=0, anti_aliasing=True
).astype(float)
mask_ravel = mask_resized.ravel()
mask_ravel[mask_ravel > 0] = 1.0
...
start_date = pd.to_datetime(datetime.fromtimestamp(times.min())).replace(
    hour=12, minute=0, second=0, microsecond=0
)
end_date = pd.to_datetime(datetime.fromtimestamp(times.max())).replace(
    hour=12, minute=0, second=0, microsecond=0
)
...
for t in date_range:  #
    t_int = t.value / 1e9
    dataset_t.samples[:, -1] = (
        t_int - model_hp.nv_samples[-1, 0]
    ) / model_hp.nv_samples[-1, 1]
    # prediction = inr.predict_loop(dataset_t, 2048, model, device=device)
    prediction, gradient = inr.predict_loop_with_time_gradient(
        dataset_t, 2048, model, device=device
    )
...
for t in date_range:  #
    t_int = t.value / 1e9
    date = t.strftime("%Y-%m-%d")
    xytz_ds.samples[:, -1] = (t_int - model_hp.nv_samples[-1, 0]) / model_hp.nv_samples[
        -1, 1
    ]
