...
def ReturnModel(
    ...
    if arch in ["default", "default-BN", "default-L"]:
        mod = Fmodel(input_size, arch, output_size, args)
    elif "skip" in arch:
        id_ = arch[5:]
        mod = SkipModel(
            input_size,
...
class Fmodel(nn.Module):
    def __init__(
        ...
        if arch in ["default", "default-BN", "default-L"]:
            self.mlp_model = gen_arch(arch, self.act, input_size, outputs)
...
class SkipModel(Fmodel):
    def __init__(
        ...
        super().__init__(input_size, "None", outputs, args)
        # if args.activation == "tanh":
        #     self.act = nn.Tanh
        # elif args.activation == "relu":
        #     self.act = nn.ReLU
    ...
    def forward(self, x):
        ...
        x = self.mlp_size0(x)
        x = self.layers(x)
        regression = self.last(x)
        # regression = torch.squeeze(regression)
        return regression


def gen_arch(arch, act, input_size, outputs):
    if arch == "default":
        mod = nn.Sequential(
...
class WiresExt(torch.nn.Module):
    def __init__(self, in_dim, out_dim, args):
        ...
        self.model = WIRES(
            ...
            args.hidden_num,
            out_dim,
            first_omega_0=args.scale,
            hidden_omega_0=args.scale,
            scale=args.width_gaussian,
        )
        # self.model = RFF(in_dim, out_dim, hidden_num, hidden_dim,
        # 'relu', feature_dim=1024, feature_scale=feature_scales[0] )
        if args.normalise_targets:
