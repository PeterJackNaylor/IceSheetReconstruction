...
for i, t in tqdm(enumerate(date_range),  total=len(date_range)):#
    ...
    imgs[:,i] = (prediction.cpu() * model_hp.nv_target[0,1] + model_hp.nv_target[0,0]).detach().numpy()[:,0]
    grad_y_minus_x[:,i] = gradient.cpu().detach()
    grad_laplacian[:,i] = laplacian.cpu().detach()
